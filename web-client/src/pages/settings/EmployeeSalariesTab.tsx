        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-[var(--paper)] rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-[var(--rule)] sticky top-0 bg-[var(--paper)]">
              <div>
                <h3 className="font-bold text-[var(--ink)] font-ui">
                  {ctcModal.annualCTC != null ? 'Revise CTC' : 'Assign CTC'}
                </h3>
                <p className="text-xs text-[var(--ink-muted)] mt-0.5">{ctcModal.employeeName}</p>
              </div>
              <button onClick={() => setCtcModal(null)} className="text-[var(--ink-muted)] hover:text-[var(--ink)]"><X size={18} /></button>
            </div>
            <form onSubmit={handleCtcSave} className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setCtcForm(f => ({ ...f, annualCTC: e.target.value }))}
                    placeholder="e.g. 600000"
                    className="register-input w-full font-mono"
                  />
                  {ctcForm.annualCTC && (
                    <p className="text-[10px] text-[var(--ink-muted)] mt-0.5">
                      Monthly: ₹{fmt(parseFloat(ctcForm.annualCTC) / 12)}
                    </p>
                  )}
                </div>
                <div>
              ...
              <div>
                ...
                <select
                  required
                  value={ctcForm.templateId}
                  onChange={e => setCtcForm(f => ({ ...f, templateId: e.target.value }))}
                  className="register-input w-full"
                >
              ...
              {/* Preview */}
              {ctcForm.annualCTC && ctcForm.templateId && (
                <div>
                  <button type="button" onClick={handlePreview} disabled={previewing} className="btn-outline text-xs">
                    {previewing ? 'Previewing...' : '🔍 Preview Breakdown'}
                  </button>
                  {previewRows.length > 0 && (
                    <div className="mt-2 border border-[var(--rule)] rounded-[4px] overflow-hidden">
                      <table className="w-full text-xs">
                        <tbody className="divide-y divide-[var(--rule)]">
                          {previewRows.filter(r => r.componentType !== 'Informational').map((r: any) => (
                            <tr key={r.componentCode} className="bg-[var(--paper)]">
                              <td className="px-3 py-1.5 text-[var(--ink)]">{r.componentName}</td>
                              <td className="px-3 py-1.5 text-[10px] text-[var(--ink-muted)]">{r.componentType}</td>
                              <td className="px-3 py-1.5 font-mono text-right font-semibold text-[var(--ink)]">
                                {r.calculationType === 'Statutory' ? '—' : `₹${fmt(r.amount)}`}
                              </td>
                            </tr>
                          ))}
                          <tr className="border-t border-[var(--rule)] font-bold bg-[var(--surface-sunken)]">
                            <td colSpan={2} className="px-3 py-1.5 text-right text-[var(--ink-muted)] text-xs">Monthly Total</td>
                            <td className="px-3 py-1.5 font-mono text-right text-[var(--accent)]">
                              ₹{fmt(previewRows.filter(r => r.componentType === 'Earning' && r.calculationType !== 'Statutory').reduce((s: number, r: any) => s + r.amount, 0))}
                            </td>
                          </tr>
                        </tbody>
                      </table>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setCtcModal(null)} className="btn-outline text-xs">Cancel</button>
                <button type="submit" disabled={ctcSaving} className="btn-primary text-xs">
                  {ctcSaving ? 'Saving...' : 'Save CTC'}
                </button>
              </div>
            </form>
          </div>
