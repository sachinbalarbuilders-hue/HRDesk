      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select 
              disabled={isBasicInfoOnly || ((lookups?.createScope === 'Department' || lookups?.createScope === 'Own Department' || lookups?.createScope === 'Reporting To' || lookups?.createScope === 'Reporting') && lookups?.departments?.length === 1)}
              value={formData.departmentId} 
              onChange={(e) => setFormData({ ...formData, departmentId: e.target.value })} 
              className={`register-input w-full ${isBasicInfoOnly || ((lookups?.createScope === 'Department' || lookups?.createScope === 'Own Department' || lookups?.createScope === 'Reporting To' || lookups?.createScope === 'Reporting') && lookups?.departments?.length === 1) ? 'opacity-80 cursor-not-allowed bg-[var(--surface-sunken)]' : ''}`}
            >
          ...
          <div>
            ...
            <select 
              disabled={isBasicInfoOnly || ((lookups?.createScope === 'Reporting To' || lookups?.createScope === 'Reporting') && lookups?.managers?.length === 1)}
              value={formData.reportingManagerId} 
              onChange={(e) => setFormData({ ...formData, reportingManagerId: e.target.value })} 
              className={`register-input w-full ${isBasicInfoOnly || ((lookups?.createScope === 'Reporting To' || lookups?.createScope === 'Reporting') && lookups?.managers?.length === 1) ? 'opacity-80 cursor-not-allowed bg-[var(--surface-sunken)]' : ''}`}
            >
