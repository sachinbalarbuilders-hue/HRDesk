    ...
    >
      {loading ? (
        <div className="p-12 flex flex-col items-center justify-center gap-3 text-[var(--ink-muted)]">
          <Calculator className="animate-spin text-[var(--gold-500)]" size={28} />
          <span className="text-xs font-data">Computing tax projections...</span>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="bg-[var(--surface-sunken)] p-4 rounded-lg border border-[var(--border)] flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
              <div>
                <span className="text-[10px] text-[var(--ink-muted)] uppercase font-data block">Annual CTC</span>
                <span className="text-sm font-semibold font-data text-[var(--ink)]">{formatCurrency(employeeData?.annualCTC)}</span>
              </div>
              <div className="hidden sm:block h-6 w-px bg-[var(--border)]" />
              <div>
                <span className="text-[10px] text-[var(--ink-muted)] uppercase font-data block">Department</span>
                <span className="text-xs font-medium text-[var(--ink)]">{employeeData?.department || '-'}</span>
              </div>
              <div className="hidden sm:block h-6 w-px bg-[var(--border)]" />
              <div>
                <span className="text-[10px] text-[var(--ink-muted)] uppercase font-data block">Current Status</span>
                <span className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium font-data ${
            ...
            <div className="flex items-center gap-2 self-start md:self-auto">
              <span className="hidden sm:inline text-xs font-medium text-[var(--ink-muted)]">Active Regime:</span>
                ...
                <button
                  ...
                  className={`px-3 py-1 text-xs font-medium rounded-md transition-all whitespace-nowrap ${
                    formData.taxRegime === 'New'
                      ? 'bg-[var(--gold-500)] text-white shadow-xs'
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-3.5 py-2 text-xs font-medium border-b-2 -mb-px transition-all cursor-pointer ${
                    isActive
            ...
            <div className="space-y-4">
              ...
              <div className={`p-4 rounded-lg border flex flex-col md:flex-row md:items-center justify-between gap-4 ${
                comparison?.recommendedRegime === 'Old'
                  ? 'bg-[var(--gold-100)]/30 border-[var(--gold-500)] text-[var(--gold-800)]'
                  : 'bg-[var(--ok-600)]/10 border-[var(--ok-600)] text-[var(--ok-700)]'
              }`}>
                <div className="flex items-start md:items-center gap-3">
                  <Sparkles size={20} className={`shrink-0 mt-0.5 md:mt-0 ${comparison?.recommendedRegime === 'Old' ? 'text-[var(--gold-600)]' : 'text-[var(--ok-600)]'}`} />
                  <div>
                    <h4 className="text-sm font-semibold">
                      Recommendation: Choose the <span className="font-bold underline">{comparison?.recommendedRegime} Tax Regime</span>
                    </h4>
                    <p className="text-xs opacity-90 mt-0.5">
                      Based on current CTC and investments, the {comparison?.recommendedRegime} Regime saves{' '}
                      <span className="font-bold font-data">{formatCurrency(comparison?.taxDifference)}</span> in annual income tax.
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className={`p-4 rounded-lg border transition-all ${
                  formData.taxRegime === 'New' ? 'border-[var(--gold-500)] shadow-xs ring-1 ring-[var(--gold-500)]/20' : 'border-[var(--border)]'
                } bg-[var(--surface)]`}>
                  <div className="flex items-center justify-between pb-3 border-b border-[var(--border)] mb-3">
                    <div>
                      <h4 className="text-sm font-semibold text-[var(--ink)]">New Regime (Sec 115BAC)</h4>
                      <p className="text-[11px] text-[var(--ink-muted)]">Default regime with simplified lower slab rates</p>
                    </div>
                    {comparison.recommendedRegime === 'New' && (
                      <span className="px-2 py-0.5 text-[10px] font-bold rounded bg-[var(--ok-600)] text-white">Recommended</span>
                    )}
                  </div>

                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between py-1 border-b border-[var(--border)]/50">
                      <span className="text-[var(--ink-muted)]">Gross Annual Income:</span>
                      <span className="font-data font-medium text-[var(--ink)]">{formatCurrency(comparison.newRegime.grossIncome)}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-[var(--border)]/50">
                      <span className="text-[var(--ink-muted)]">Standard Deduction:</span>
                      <span className="font-data font-medium text-[var(--ok-600)]">-{formatCurrency(comparison.newRegime.standardDeduction)}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-[var(--border)]/50">
                      <span className="text-[var(--ink-muted)]">Other Deductions / 80C:</span>
                      <span className="font-data text-[var(--ink-muted)]">N/A (Disallowed)</span>
                    </div>
                    <div className="flex justify-between py-1 font-semibold text-[var(--ink)]">
                      <span>Net Taxable Income:</span>
                      <span className="font-data">{formatCurrency(comparison.newRegime.netTaxableIncome)}</span>
                    </div>
                    <div className="flex justify-between py-1 text-[var(--ink-muted)]">
                      <span>Tax Before Rebate:</span>
                      <span className="font-data">{formatCurrency(comparison.newRegime.TaxBeforeRebate)}</span>
                    </div>
                    {comparison.newRegime.Section87ARebate > 0 && (
                      <div className="flex justify-between py-1 text-[var(--ok-600)]">
                        <span>Sec 87A Rebate:</span>
                        <span className="font-data">-{formatCurrency(comparison.newRegime.Section87ARebate)}</span>
                      </div>
                    )}
                    <div className="flex justify-between py-1 text-[var(--ink-muted)]">
                      <span>Health & Education Cess (4%):</span>
                      <span className="font-data">{formatCurrency(comparison.newRegime.HealthAndEducationCess)}</span>
                    </div>
                    <div className="flex justify-between pt-2 border-t-2 border-[var(--border)] text-sm font-bold text-[var(--ink)]">
                      <span>Total Annual Tax:</span>
                      <span className="font-data text-[var(--err-600)]">{formatCurrency(comparison.newRegime.totalAnnualTax)}</span>
                    </div>
                    <div className="flex justify-between py-1 text-[11px] text-[var(--ink-muted)] bg-[var(--surface-sunken)] p-2 rounded">
                      <span>Monthly Projected TDS:</span>
                      <span className="font-data font-bold text-[var(--ink)]">{formatCurrency(comparison.newRegime.estimatedMonthlyTds)} / mo</span>
                    </div>
                  </div>
                </div>
                ...
                <div className={`p-4 rounded-lg border transition-all ${
                  ...
                  <div className="flex items-center justify-between pb-3 border-b border-[var(--border)] mb-3">
                    <div>
                      <h4 className="text-sm font-semibold text-[var(--ink)]">Old Regime</h4>
                      <p className="text-[11px] text-[var(--ink-muted)]">Standard rates with 80C, 80D, HRA & Sec 24 exemptions</p>
                    </div>
                    {comparison.recommendedRegime === 'Old' && (
                      <span className="px-2 py-0.5 text-[10px] font-bold rounded bg-[var(--gold-500)] text-white">Recommended</span>
                    )}
                  </div>
                  ...
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between py-1 border-b border-[var(--border)]/50">
                      <span className="text-[var(--ink-muted)]">Gross Annual Income:</span>
                      <span className="font-data font-medium text-[var(--ink)]">{formatCurrency(comparison.oldRegime.grossIncome)}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-[var(--border)]/50">
                      <span className="text-[var(--ink-muted)]">Standard Deduction:</span>
                      <span className="font-data font-medium text-[var(--ok-600)]">-{formatCurrency(comparison.oldRegime.standardDeduction)}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-[var(--border)]/50">
                      <span className="text-[var(--ink-muted)]">HRA Exemption:</span>
                      <span className="font-data font-medium text-[var(--ok-600)]">-{formatCurrency(comparison.oldRegime.HraExemption)}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-[var(--border)]/50">
                      <span className="text-[var(--ink-muted)]">Section 80C Deductions:</span>
                      <span className="font-data font-medium text-[var(--ok-600)]">-{formatCurrency(comparison.oldRegime.Section80CDeduction)}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-[var(--border)]/50">
                      <span className="text-[var(--ink-muted)]">Section 80D (Medical):</span>
                      <span className="font-data font-medium text-[var(--ok-600)]">-{formatCurrency(comparison.oldRegime.Section80DDeduction)}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-[var(--border)]/50">
                      <span className="text-[var(--ink-muted)]">Home Loan Interest (Sec 24):</span>
                      <span className="font-data font-medium text-[var(--ok-600)]">-{formatCurrency(comparison.oldRegime.Section24Deduction)}</span>
                    </div>
                    <div className="flex justify-between py-1 font-semibold text-[var(--ink)]">
                      <span>Net Taxable Income:</span>
                      <span className="font-data">{formatCurrency(comparison.oldRegime.netTaxableIncome)}</span>
                    </div>
                    <div className="flex justify-between py-1 text-[var(--ink-muted)]">
                      <span>Tax Before Rebate:</span>
                      <span className="font-data">{formatCurrency(comparison.oldRegime.TaxBeforeRebate)}</span>
                    </div>
                    {comparison.oldRegime.Section87ARebate > 0 && (
                      <div className="flex justify-between py-1 text-[var(--ok-600)]">
                        <span>Sec 87A Rebate:</span>
                        <span className="font-data">-{formatCurrency(comparison.oldRegime.Section87ARebate)}</span>
                      </div>
                    )}
                    <div className="flex justify-between py-1 text-[var(--ink-muted)]">
                      <span>Health & Education Cess (4%):</span>
                      <span className="font-data">{formatCurrency(comparison.oldRegime.HealthAndEducationCess)}</span>
                    </div>
                    <div className="flex justify-between pt-2 border-t-2 border-[var(--border)] text-sm font-bold text-[var(--ink)]">
                      <span>Total Annual Tax:</span>
                      <span className="font-data text-[var(--err-600)]">{formatCurrency(comparison.oldRegime.totalAnnualTax)}</span>
                    </div>
                    <div className="flex justify-between py-1 text-[11px] text-[var(--ink-muted)] bg-[var(--surface-sunken)] p-2 rounded">
                      <span>Monthly Projected TDS:</span>
                      <span className="font-data font-bold text-[var(--ink)]">{formatCurrency(comparison.oldRegime.estimatedMonthlyTds)} / mo</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'sec80c' && (
            <div className="space-y-4">
              <div className="bg-[var(--surface-sunken)] p-3 rounded-lg border border-[var(--border)] flex items-center justify-between">
                <div>
                  <span className="text-xs font-semibold text-[var(--ink)]">Section 80C Deductions Limit</span>
                  <p className="text-[11px] text-[var(--ink-muted)]">Maximum permissible deduction across all 80C components is ₹1,50,000</p>
                </div>
                <div className="text-right font-data">
                  <span className="text-sm font-bold text-[var(--ink)]">{formatCurrency(eligible80C)}</span>
                  <span className="text-xs text-[var(--ink-muted)]"> / ₹1,50,000</span>
                  <div className="w-32 bg-[var(--border)] h-1.5 rounded-full overflow-hidden mt-1">
                    <div
                      className="bg-[var(--gold-500)] h-full transition-all"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div className="space-y-1">
                  <label className="font-medium text-[var(--ink)]">Employee PF Contribution (EPF)</label>
                  <Input type="number" name="sec80C_EPF" value={formData.sec80C_EPF} onChange={handleChange} min="0" />
                  <p className="text-[10px] text-[var(--ink-muted)]">Auto-calculated employee contribution to Provident Fund</p>
                </div>
                <div className="space-y-1">
                  <label className="font-medium text-[var(--ink)]">Public Provident Fund (PPF)</label>
                  <Input type="number" name="sec80C_PPF" value={formData.sec80C_PPF} onChange={handleChange} min="0" />
                </div>
                <div className="space-y-1">
                  <label className="font-medium text-[var(--ink)]">ELSS Tax Saving Mutual Funds</label>
                  <Input type="number" name="sec80C_ELSS" value={formData.sec80C_ELSS} onChange={handleChange} min="0" />
                </div>
                <div className="space-y-1">
                  <label className="font-medium text-[var(--ink)]">Life Insurance Premium (LIC / Term)</label>
                  <Input type="number" name="sec80C_LifeInsurance" value={formData.sec80C_LifeInsurance} onChange={handleChange} min="0" />
                </div>
                <div className="space-y-1">
                  <label className="font-medium text-[var(--ink)]">Children Tuition Fees (Max 2 children)</label>
                  <Input type="number" name="sec80C_TuitionFees" value={formData.sec80C_TuitionFees} onChange={handleChange} min="0" />
                </div>
                <div className="space-y-1">
                  <label className="font-medium text-[var(--ink)]">Home Loan Principal Repayment</label>
                ...
                <div className="space-y-1 md:col-span-2">
                  <label className="font-medium text-[var(--ink)]">Other 80C Instruments (NSC, Sukanya Samriddhi, 5+ Yr Tax Saving FD)</label>
          ...
          {activeTab === 'sec80d' && (
            <div className="space-y-4">
              <div className="bg-[var(--surface-sunken)] p-3 rounded-lg border border-[var(--border)] text-xs text-[var(--ink-muted)]">
                Health Insurance Premiums under Section 80D: Self/Family (Max ₹25,000) + Parents (Max ₹25,000, or ₹50,000 if senior citizen).
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div className="space-y-1">
                  <label className="font-medium text-[var(--ink)]">Mediclaim Premium (Self, Spouse & Children)</label>
                  <Input type="number" name="sec80D_SelfFamily" value={formData.sec80D_SelfFamily} onChange={handleChange} min="0" />
                  <p className="text-[10px] text-[var(--ink-muted)]">Max deduction: ₹25,000</p>
                </div>

                <div className="space-y-1">
                  <label className="font-medium text-[var(--ink)]">Preventive Health Check-up</label>
                  <Input type="number" name="sec80D_PreventiveCheckup" value={formData.sec80D_PreventiveCheckup} onChange={handleChange} min="0" />
                  <p className="text-[10px] text-[var(--ink-muted)]">Capped at ₹5,000 across self and family</p>
                </div>

                <div className="space-y-1">
                  <label className="font-medium text-[var(--ink)]">Parents Medical Insurance Premium</label>
                ...
                <div className="flex items-center gap-2 pt-6">
                  <input
                    ...
                    className="h-4 w-4 rounded border-[var(--border)] text-[var(--gold-500)] focus:ring-[var(--gold-500)]"
                  />
                  <label htmlFor="sec80D_ParentsSeniorCitizen" className="text-xs font-medium text-[var(--ink)] cursor-pointer">
          ...
          {activeTab === 'hra' && (
            <div className="space-y-4">
              <div className="p-3 bg-[var(--surface-sunken)] rounded-lg border border-[var(--border)] text-xs text-[var(--ink-muted)]">
              ...
              <div className="border border-[var(--border)] p-4 rounded-lg space-y-4">
                <h4 className="text-xs font-bold uppercase tracking-wider text-[var(--ink)] flex items-center gap-2">
                  <Home size={14} className="text-[var(--gold-500)]" /> House Rent Allowance (HRA) Exemption
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  <div className="space-y-1">
                    <label className="font-medium text-[var(--ink)]">Annual Rent Paid (₹)</label>
                    <Input type="number" name="annualRentPaid" value={formData.annualRentPaid} onChange={handleChange} min="0" />
                  </div>
                  <div className="space-y-1">
                    <label className="font-medium text-[var(--ink)]">Rental City Category</label>
                    <select
                      name="rentalCityType"
                      value={formData.rentalCityType}
                      onChange={handleChange}
                      className="w-full px-3 py-2 bg-[var(--surface)] border border-[var(--border)] rounded-md text-xs font-medium"
                    >
                  ...
                  <div className="space-y-1">
                    <label className="font-medium text-[var(--ink)]">Landlord Name</label>
                    <Input type="text" name="landlordName" value={formData.landlordName} onChange={handleChange} placeholder="Required if rent > ₹1L" />
                  </div>
                  <div className="space-y-1">
                    <label className="font-medium text-[var(--ink)]">Landlord PAN</label>
          ...
          {activeTab === 'other' && (
            <div className="space-y-4 text-xs">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="font-medium text-[var(--ink)]">Section 80CCD(1B) - NPS Additional Contribution</label>
                  <Input type="number" name="sec80CCD_NPS" value={formData.sec80CCD_NPS} onChange={handleChange} min="0" />
                  <p className="text-[10px] text-[var(--ink-muted)]">Exclusive deduction up to ₹50,000 over and above 80C</p>
                </div>

                <div className="space-y-1">
                  <label className="font-medium text-[var(--ink)]">Section 80E - Education Loan Interest</label>
          ...
          {activeTab === 'proofs' && (
            <div className="space-y-6 text-xs">
              ...
              <div>
                ...
                {proofs.length === 0 ? (
                  <div className="p-8 text-center text-[var(--ink-muted)] border border-dashed rounded-lg">
                    ...
                    {proofs.map((p, idx) => (
                      <div key={idx} className="flex items-center justify-between p-3 rounded-lg border bg-[var(--surface)]">
                        <div className="flex items-center gap-3">
                          <ShieldCheck className="text-[var(--gold-500)]" size={18} />
                          <div>
                        ...
                        <div className="flex items-center gap-2">
                          <a 
                            ...
                            target="_blank" 
                            rel="noreferrer"
                            className="px-3 py-1 bg-[var(--surface-sunken)] hover:bg-[var(--rule)] rounded text-xs font-medium transition-colors"
                          >
                            View
                          </a>
                          {formData.status !== 'Approved' && (
                            <button
                              type="button"
                              className="px-3 py-1 bg-[var(--err-500)]/10 text-[var(--err-600)] hover:bg-[var(--err-500)]/20 rounded text-xs font-medium transition-colors"
                              onClick={async () => {
          ...
          <div className="pt-4 border-t border-[var(--border)] flex items-center justify-between">
              ...
              {canApprove && (
                <>
                  <button
                    ...
                    className="px-3 py-2 text-xs font-medium rounded-md bg-[var(--ok-600)] text-white hover:opacity-90 transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-2 text-xs font-medium rounded-md border border-[var(--err-600)] text-[var(--err-600)] hover:bg-[var(--err-600)]/10 transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                  >
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 text-xs font-medium rounded-md border border-[var(--border)] text-[var(--ink-muted)] hover:text-[var(--ink)] cursor-pointer"
              >
                Close
              </button>
              <button
                ...
                className="px-4 py-2 text-xs font-medium rounded-md border border-[var(--border)] text-[var(--ink)] bg-[var(--surface)] hover:bg-[var(--surface-sunken)] cursor-pointer disabled:opacity-50"
              >
                Save as Draft
              </button>
              <button
                ...
                className="btn-primary px-4 py-2 text-xs font-medium flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
                Submit Declaration
              </button>
            </div>
