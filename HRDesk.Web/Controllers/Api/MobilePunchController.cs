...
[Authorize]
public class MobilePunchController : ControllerBase
{
    private readonly BiometricAttendanceDbContext _db;
    private readonly AttendanceSummaryService _attendanceSummaryService;
    private readonly AttendanceDialService _dialService;
    ...
    public MobilePunchController(
        ...
        AttendanceDialService dialService,
    ...
    public async Task<IActionResult> GetDayDetails([FromQuery] int employeeId, [FromQuery] string date)
    {
        ...
        // Delegate dial timeline, overtime, and segment calculations to AttendanceDialService (Clean Architecture)
        var dial = _dialService.ComputeShiftDial(
            dailySummary?.ShiftStart,
            dailySummary?.ShiftEnd,
            effectiveInTime,
            effectiveOutTime,
            dailySummary?.BreakMinutes ?? 0,
            dailySummary?.WorkMinutes ?? 0,
            recordDate
        );
        ...
        return Ok(new
        {
            ...
            shift = dailySummary?.ShiftName != null ? new
            {
                ...
                start = dailySummary.ShiftStart?.ToString("HH:mm"),
                end = dailySummary.ShiftEnd?.ToString("HH:mm"),
                colorCode = dailySummary.ShiftColor
            } : null,
            dial = new
            {
                shiftTotalMinutes = dial.ShiftTotalMinutes,
                shiftStart = dial.ShiftStart,
                shiftEnd = dial.ShiftEnd,
                regularWorkMinutes = dial.RegularWorkMinutes,
                overtimeMinutes = dial.OvertimeMinutes,
                breakMinutes = dial.BreakMinutes,
                remainingMinutes = dial.RemainingMinutes,
                effectiveWorkMinutes = dial.EffectiveWorkMinutes,
                isOvertime = dial.IsOvertime,
                isShiftInProgress = dial.IsShiftInProgress,
                inProgressElapsedMinutes = dial.InProgressElapsedMinutes,
                isAfterHours = dial.IsAfterHours,
                lateArrivalMinutes = dial.LateArrivalMinutes,
                statusBadge = dial.StatusBadge,
                subtitle = dial.Subtitle,
                totalWorkFormatted = dial.TotalWorkFormatted,
                segments = dial.Segments.Select(s => new
                {
                    id = s.Id,
                    label = s.Label,
                    minutes = s.Minutes,
